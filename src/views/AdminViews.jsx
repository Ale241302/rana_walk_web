...
const StatusBadge = ({ status }) => {
    const styles = {
        active: 'bg-[#75CBB3]/20 text-[#066383]',
        inactive: 'bg-slate-500/20 text-slate-400',
        pending: 'bg-[#C5A96A]/20 text-[#C5A96A]',
        completed: 'bg-[#75CBB3]/20 text-[#066383]',
        shipped: 'bg-[#066383]/20 text-[#066383]',
        processing: 'bg-[#066383]/20 text-[#066383]',
        paid: 'bg-[#75CBB3]/20 text-[#066383]',
    };
    ...
    <div className="py-12 animate-fadeIn">
        ...
        <div className="container mx-auto px-6 max-w-7xl">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-10">
                ...
                <button className="mt-4 md:mt-0 bg-[#75CBB3] text-[#013A57] px-6 py-3 rounded-full font-black uppercase tracking-widest text-xs">+ Nuevo Cliente</button>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                ...
                <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm"><p className="text-slate-400 text-xs font-bold uppercase mb-2">Activos</p><p className="text-3xl font-black text-[#75CBB3]">{clientsData.filter(c => c.status === 'active').length}</p></div>
    ...
    <div className="py-12 animate-fadeIn">
        <div className="container mx-auto px-6 max-w-7xl">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-10">
                ...
                <button className="mt-4 md:mt-0 bg-[#75CBB3] text-[#013A57] px-6 py-3 rounded-full font-black uppercase tracking-widest text-xs">+ Nuevo Distribuidor</button>
            </div>
    ...
    <div className="py-12 animate-fadeIn">
        <div className="container mx-auto px-6 max-w-7xl">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-10">
                ...
                <button className="mt-4 md:mt-0 bg-[#75CBB3] text-[#013A57] px-6 py-3 rounded-full font-black uppercase tracking-widest text-xs">+ Nuevo Producto</button>
            </div>
        ...
        <div className="py-12 animate-fadeIn">
            <div className="container mx-auto px-6 max-w-7xl">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm"><p className="text-slate-400 text-xs font-bold uppercase mb-2">Pendientes</p><p className="text-3xl font-black text-[#C5A96A]">{ordersData.filter(o => o.status === 'pending').length}</p></div>
                    <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm"><p className="text-slate-400 text-xs font-bold uppercase mb-2">Completados</p><p className="text-3xl font-black text-[#75CBB3]">{ordersData.filter(o => o.status === 'completed').length}</p></div>
                ...
                <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                    <table className="w-full">
                        ...
                        <tbody>{ordersData.map(o => (<tr key={o.id} className="border-b border-slate-100 hover:bg-slate-50"><td className="px-6 py-4 font-black text-[#013A57]">{o.id}</td><td className="px-6 py-4 font-bold text-slate-700">{o.client}</td><td className="px-6 py-4">{!o.distributor && !o.subDistributor ? <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase bg-slate-500/20 text-slate-400">Propio</span> : o.subDistributor ? <div><span className="px-3 py-1 rounded-full text-[10px] font-black uppercase bg-[#066383]/20 text-[#066383]">{o.distributor}</span><p className="text-[9px] text-[#7B2DBF] mt-1">vía {o.subDistributor}</p></div> : <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase bg-[#066383]/20 text-[#066383]">{o.distributor}</span>}</td><td className="px-6 py-4 text-slate-500"><Calendar className="w-4 h-4 inline mr-1" />{o.date}</td><td className="px-6 py-4 font-black text-[#75CBB3]">₡{o.total.toLocaleString()}</td><td className="px-6 py-4"><StatusBadge status={o.status} /></td><td className="px-6 py-4"><StatusBadge status={o.paymentStatus} /></td><td className="px-6 py-4"><button onClick={() => handleTrack(o.id, o.tracking)} className="flex items-center gap-2 bg-[#066383] hover:bg-[#013A57] text-white px-4 py-2 rounded-full font-bold text-xs uppercase transition-colors"><Navigation className="w-4 h-4" />Rastrear</button></td></tr>))}</tbody>
                    </table>
                </div>
                <div className="mb-12">
                </div>
